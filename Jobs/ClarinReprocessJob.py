...
class ClarinReprocessJob(Job):
    ...
    def launch(self):
        ...
        for collection in self.cursors_by_collection:
            len_collection = len(self.cursors_by_collection[collection])
            print("len_collection : ", len_collection)



    def store(self, file_commit = False):
        if file_commit:
            print('Stashing results into a file')
            # TODO: Stash to file
        else:
            print('Stashing results into SQL DataBase')
        for collection in self.cursors_by_collection:
            # os.makedirs(".test/" + collection, exist_ok = True)
            for doc in self.cursors_by_collection[collection]:  # The cursor is iterable
                print(collection, list(doc.keys()))
                print('\t_id', doc['_id'])
                print('\tcaptureDatetime', doc['captureDatetime'])
                print('\tjobName', doc['jobName'])
                
                print('\trawData.request_text (length)', len(doc['rawData']['request_text']))
                print('\trawData.request_reason', doc['rawData']['request_reason'])
                print('\trawData.request_status_code', doc['rawData']['request_status_code'])
                print('\trawData.request_apparent_encoding', doc['rawData']['request_apparent_encoding'])
